      ...
      <ProtectedRoute>
        <div className="container mx-auto p-12 text-center max-w-md space-y-4">
          ...
          <Link href="/job-descriptions">
            <Button size="sm">Back to Job Descriptions</Button>
          </Link>
        </div>
      </ProtectedRoute>
    );
      ...
      <div className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-4xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-foreground">{jd.title}</h1>
              <p className="text-xs text-muted-foreground">{jd.company || "Target Position Criteria"}</p>
            </div>
          </div>
